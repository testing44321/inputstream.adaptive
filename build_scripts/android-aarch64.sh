...
./make-standalone-toolchain.sh --force --install-dir=$HOME/tools/toolchain --platform=android-21 --toolchain=aarch64-linux-android
...
if [[ $version == "leia" ]]; then
    ...
    sed -i -E "s/version=\"(.+)\"/version=\"\1-matrix.1\"/" $HOME/inputstream.adaptive/inputstream.adaptive.testing/addon.xml.in
fi
